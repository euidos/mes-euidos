...
| 빌드 | **Frappe 내장 번들러** (`*.bundle.js`) — 별도 빌드 파이프라인·node 도구체인 없음 |
| 서버 호출 | `frappe.call` → `mes_demo.api.<모듈>.<함수>` |
...
천일이 쓴 `www/` 독립 라우트 대신 Desk 페이지에 마운트하는 점만 다름 (`[[decision-ui-architecture]]`).
...
별도 빌드 파이프라인 없이 Frappe 번들러만 씀 — 이 점이 차용할 방법론.
