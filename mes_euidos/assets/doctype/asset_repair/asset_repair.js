...
frappe.ui.form.on("Asset Repair", {
	setup: function (frm) {
		...
		frm.set_query("purchase_invoice", "invoices", function () {
			return {
				query: "mes_euidos.assets.doctype.asset_repair.asset_repair.get_purchase_invoice",
				filters: {
					company: frm.doc.company,
				},
			};
		...
		frm.set_query("expense_account", "invoices", function (doc, cdt, cdn) {
			...
			return {
				query: "mes_euidos.assets.doctype.asset_repair.asset_repair.get_expense_accounts",
				filters: {
	...
	repair_status: (frm) => {
		if (frm.doc.completion_date && frm.doc.repair_status == "Completed") {
			frappe.call({
				method: "mes_euidos.assets.doctype.asset_repair.asset_repair.get_downtime",
				args: {
...
frappe.ui.form.on("Asset Repair Purchase Invoice", {
	...
	expense_account: function (frm, cdt, cdn) {
		...
		frappe.call({
			method: "mes_euidos.assets.doctype.asset_repair.asset_repair.get_unallocated_repair_cost",
			args: {
				purchase_invoice: row.purchase_invoice,
