...
frappe.provide("mes_euidos.queries");
$.extend(mes_euidos.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "mes_euidos.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "mes_euidos.controllers.queries.bom" };
	},

	task: function () {
		return { query: "mes_euidos.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "mes_euidos.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "mes_euidos.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
mes_euidos.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
mes_euidos.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = mes_euidos.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "mes_euidos.controllers.queries.warehouse_query" });
