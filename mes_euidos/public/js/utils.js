...
frappe.provide("mes_euidos");
frappe.provide("mes_euidos.utils");
frappe.provide("mes_euidos.stock.utils");
...
$.extend(mes_euidos, {
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (mes_euidos.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", mes_euidos.last_selected_company);
			}
		}
	},
...
$.extend(mes_euidos.utils, {
	set_party_dashboard_indicators: function (frm) {
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					mes_euidos.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return frappe.call({
				method: "mes_euidos.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "mes_euidos.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		...
		if (
			...
			frappe.call({
				method: "mes_euidos.accounts.utils.get_fiscal_year",
...
mes_euidos.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
			...
			{ fieldtype: "Section Break", label: __("Items") },
			{
				...
				fields: [
					...
					},
					{
						...
						get_query: (e) => {
							return {
								query: "mes_euidos.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
...
mes_euidos.utils.update_child_items = function (opts) {
	const frm = opts.frm;
	...
	let dialog = new frappe.ui.Dialog({
		...
		update_items: function () {
			...
			frappe.call({
				method: "mes_euidos.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
...
mes_euidos.utils.map_current_doc = function (opts) {
	function _map() {
	...
	$.each(frappe.boot.service_level_agreement_doctypes, function (_i, d) {
		frappe.ui.form.on(d, {
			onload: function (frm) {
				...
				frappe.call({
					method: "mes_euidos.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
						doctype: frm.doc.doctype,
...
$.extend(mes_euidos.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new mes_euidos.utils.BarcodeScanner({ frm: frm });
