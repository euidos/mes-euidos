frappe.provide("mes_euidos.setup");
...
frappe.setup.on("before_load", function () {
	if (
		...
		frappe.boot.setup_wizard_completed_apps.includes("mes_euidos")
	) {
		return;
	}

	mes_euidos.setup.slides_settings.map(frappe.setup.add_slide);
});

mes_euidos.setup.slides_settings = [
	{
		...
		apply_industry_modules: function (slide) {
			...
			let modules = mes_euidos.setup.industry_modules[industry] || ["accounting"];
	...
	},
	{
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = mes_euidos.setup.fiscal_years[country];
		...
		load_chart_of_accounts: function (slide) {
			...
			if (country) {
				frappe.call({
					method: "mes_euidos.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: "mes_euidos.accounts.utils.get_coa",
				args: {
...
mes_euidos.setup.industry_modules = {
