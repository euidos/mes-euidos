...
mes_euidos.buying = {
	setup_buying_controller: function () {
		mes_euidos.buying.BuyingController = class BuyingController extends mes_euidos.TransactionController {
			setup() {
				super.setup();
			...
			onload(doc, cdt, cdn) {
				...
				if (this.frm.get_field("shipping_address")) {
					this.frm.set_query("shipping_address", () => {
						...
						} else return mes_euidos.queries.company_address_query(this.frm.doc);
					});
				}

				if (this.frm.get_field("dispatch_address")) {
					this.frm.set_query("dispatch_address", () => {
						return mes_euidos.queries.address_query(this.frm.doc);
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query("supplier", mes_euidos.queries.supplier);
				me.frm.set_query("contact_person", mes_euidos.queries.contact_query);
				me.frm.set_query("supplier_address", mes_euidos.queries.address_query);

				me.frm.set_query("billing_address", mes_euidos.queries.company_address_query);
				mes_euidos.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);
				...
				this.frm.set_query("manufacturer", "items", function (doc, cdt, cdn) {
					...
					return {
						query: "mes_euidos.controllers.queries.item_manufacturer_query",
			...
			supplier() {
				var me = this;
				mes_euidos.utils.get_party_details(this.frm, null, null, function () {
					me.apply_price_list();
				});
			}
			...
			company() {
				...
				frappe.call({
					method: "mes_euidos.setup.doctype.company.company.get_billing_shipping_address",
					args: {
				...
				mes_euidos.utils.set_letter_head(this.frm);
			}

			supplier_address() {
				mes_euidos.utils.get_address_display(this.frm);
				mes_euidos.utils.set_taxes_from_address(
					this.frm,
					"supplier_address",
					"supplier_address",
					"supplier_address"
			...
			warehouse(doc, cdt, cdn) {
				...
				if (item.item_code && item.warehouse) {
					return this.frm.call({
						method: "mes_euidos.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(
					__("Product Bundle"),
					function () {
						mes_euidos.buying.get_items_from_product_bundle(me.frm);
					},
					__("Get Items From")
				);
			}
			...
			shipping_address() {
				var me = this;
				mes_euidos.utils.get_address_display(
					this.frm,
					"shipping_address",
			...
			manufacturer(doc, cdt, cdn) {
				...
				if (row.manufacturer) {
					frappe.call({
						method: "mes_euidos.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
			...
			add_serial_batch_bundle(doc, cdt, cdn) {
				...
				frappe.db.get_value("Item", item.item_code, fields).then((r) => {
					if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
						...
						new mes_euidos.SerialBatchPackageSelector(me.frm, item, (r) => {
							if (r) {
...
mes_euidos.buying.link_to_mrs = function (frm) {
	frappe.call({
...
mes_euidos.buying.get_default_bom = function (frm) {
	$.each(frm.doc["items"] || [], function (i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "mes_euidos.stock.get_item_details.get_default_bom",
				args: {
...
mes_euidos.buying.get_items_from_product_bundle = function (frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args) {
			...
			return frappe.call({
				type: "GET",
				method: "mes_euidos.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
...
mes_euidos.buying.prevent_past_schedule_dates = function (frm) {
