frappe.provide("mes_euidos.financial_statements");
...
mes_euidos.financial_statements = {
	filters: get_filters(),
	baseData: null,
	...
	formatter: function (value, row, column, data, default_formatter, filter) {
		if (mes_euidos.financial_statements.is_blank_row(data)) return "";

		const report_params = [value, row, column, data, default_formatter, filter];
		// Growth/Margin
		if (mes_euidos.financial_statements._is_special_view(column, data))
			return mes_euidos.financial_statements._format_special_view(...report_params);

		if (get_filter_value("report_template"))
			return mes_euidos.financial_statements._format_custom_report(...report_params);
		else return mes_euidos.financial_statements._format_standard_report(...report_params);
	},
	...
	_format_custom_report: function (value, row, column, data, default_formatter, filter) {
		const columnInfo = mes_euidos.financial_statements._parse_column_info(column.fieldname, data);
		const formatting = mes_euidos.financial_statements._get_formatting_for_column(data, columnInfo);

		if (columnInfo.isAccount) {
			return mes_euidos.financial_statements._format_custom_account_column(
				value,
				data,
				formatting,
				column,
			...
			return mes_euidos.financial_statements._format_custom_value_column(
				value,
				data,
				formatting,
				column,
	...
	_format_custom_account_column: function (value, data, formatting, column, default_formatter, row) {
		...
		if (should_link_to_ledger) {
			...
			column.link_onclick =
				"mes_euidos.financial_statements.open_general_ledger(" + JSON.stringify(glData) + ")";
		...
		// Style
		return mes_euidos.financial_statements._style_custom_value(formattedValue, formatting, null);
	},
	...
	_format_custom_value_column: function (value, data, formatting, column, default_formatter, row) {
		...
		return mes_euidos.financial_statements._style_custom_value(formattedValue, formatting, value);
	},
	...
	_format_standard_report: function (value, row, column, data, default_formatter, filter) {
		if (data && column.fieldname == mes_euidos.financial_statements.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"mes_euidos.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		mes_euidos.financial_statements.filters = get_filters();

		let fiscal_year = mes_euidos.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: mes_euidos.get_presentation_currency_list(),
	...
	let fiscal_year = mes_euidos.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
	if (fiscal_year) {
