frappe.provide("mes_euidos.stock_reservation");

$.extend(mes_euidos.stock_reservation, {
	make_entries(frm, table_name) {
		mes_euidos.stock_reservation.setup(frm, table_name);
	},

	setup(frm, table_name) {
		let parms = mes_euidos.stock_reservation.get_parms(frm, table_name);

		mes_euidos.stock_reservation.dialog = new frappe.ui.Dialog({
			title: __("Stock Reservation"),
			size: "extra-large",
			fields: mes_euidos.stock_reservation.get_dialog_fields(frm, parms),
			primary_action_label: __("Reserve Stock"),
			primary_action: () => {
				mes_euidos.stock_reservation.reserve_stock(frm, table_name, parms);
			},
		});

		mes_euidos.stock_reservation.render_items(frm, parms);
	},
	...
	get_parms(frm, table_name) {
		...
		params["method"] = {
			"Sales Order": "delivered_qty",
			"Production Plan":
				"mes_euidos.manufacturing.doctype.production_plan.production_plan.make_stock_reservation_entries",
			"Work Order":
				"mes_euidos.manufacturing.doctype.work_order.work_order.make_stock_reservation_entries",
		}[frm.doc.doctype];
	...
	get_dialog_fields(frm, parms) {
		let fields = mes_euidos.stock_reservation.fields || [];
		let qty_field = parms.qty_field;
		let dialog = mes_euidos.stock_reservation.dialog;
	...
	unreserve_stock(frm) {
		mes_euidos.stock_reservation.get_stock_reservation_entries(frm.doctype, frm.docname).then((r) => {
			if (!r.exc && r.message) {
				if (r.message.length > 0) {
					mes_euidos.stock_reservation.prepare_for_cancel_sre_entries(frm, r.message);
				} else {
			...
			fields: [
				{
					...
					in_place_edit: true,
					data: [],
					fields: mes_euidos.stock_reservation.get_fields_for_cancel(),
				},
			],
			primary_action_label: __("Unreserve Stock"),
			primary_action: () => {
				mes_euidos.stock_reservation.cancel_stock_reservation(dialog, frm);
			},
		});
	...
	cancel_stock_reservation(dialog, frm) {
		...
		let method = "mes_euidos.manufacturing.doctype.work_order.work_order.cancel_stock_reservation_entries";

		if (frm.doc.doctype === "Production Plan") {
			method =
				"mes_euidos.manufacturing.doctype.production_plan.production_plan.cancel_stock_reservation_entries";
		}
