...
mes_euidos.LeadController = class LeadController extends frappe.ui.form.Controller {
	setup() {
	...
	add_lead_to_prospect(frm) {
		frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "mes_euidos.crm.doctype.lead.lead.add_lead_to_prospect",
					args: {
						lead: frm.doc.name,
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.crm.doctype.lead.lead.make_customer",
			frm: this.frm,
		});
	}
	...
	make_quotation() {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.crm.doctype.lead.lead.make_quotation",
			frm: this.frm,
		});
	}
...
if (cur_frm) {
	extend_cscript(cur_frm.cscript, new mes_euidos.LeadController({ frm: cur_frm }));
}
