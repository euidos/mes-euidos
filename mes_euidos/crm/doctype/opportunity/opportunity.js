...
frappe.provide("mes_euidos.crm");
mes_euidos.pre_sales.set_as_lost("Opportunity");
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function (frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			mes_euidos.utils.get_party_details(frm);
		} else if (frm.doc.opportunity_from == "Lead") {
			mes_euidos.utils.map_current_doc({
				method: "mes_euidos.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function (frm, cdt, cdn) {
		mes_euidos.utils.get_address_display(frm, "customer_address", "address_display", false);
	},

	contact_person: mes_euidos.utils.get_contact_details,
	...
	refresh: function (frm) {
		...
		if (!frm.is_new() && doc.status !== "Lost") {
			...
			let company_currency = mes_euidos.get_currency(frm.doc.company);
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
mes_euidos.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		me.frm.set_query("customer_address", mes_euidos.queries.address_query);
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query("party_name", mes_euidos.queries["lead"]);
		} else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query("party_name", mes_euidos.queries["customer"]);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.crm.doctype.opportunity.opportunity.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}

	show_notes() {
		const crm_notes = new mes_euidos.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new mes_euidos.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new mes_euidos.crm.Opportunity({ frm: cur_frm }));

cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "mes_euidos.crm.doctype.opportunity.opportunity.get_item_details",
