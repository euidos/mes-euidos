...
from mes_euidos.accounts.doctype.account_closing_balance.account_closing_balance import (
...
class ProcessPeriodClosingVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		from mes_euidos.accounts.doctype.process_period_closing_voucher_detail.process_period_closing_voucher_detail import (
	...
	if normal_balances := (
		...
		if not is_scheduler_inactive():
			for x in normal_balances:
				...
				frappe.enqueue(
					method="mes_euidos.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.process_individual_date",
					queue="long",
					timeout=timeout,
					is_async=True,
	...
	if to_process := (
		...
		if total_no_of_dates == completed:
			...
			if not is_job_running(job_name):
				frappe.enqueue(
					method="mes_euidos.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.summarize_and_post_ledger_entries",
					queue="long",
					timeout=timeout,
					is_async=True,
					job_name=job_name,
