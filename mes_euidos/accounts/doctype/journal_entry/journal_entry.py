...
import mes_euidos
from mes_euidos.accounts.deferred_revenue import get_deferred_booking_accounts
from mes_euidos.accounts.doctype.invoice_discounting.invoice_discounting import (
...
from mes_euidos.accounts.doctype.tax_withholding_entry.tax_withholding_entry import JournalTaxWithholding
from mes_euidos.accounts.general_ledger import validate_opening_entry_against_pcv
from mes_euidos.accounts.party import get_party_account
...
from mes_euidos.setup.utils import get_exchange_rate as _get_exchange_rate
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from mes_euidos.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def validate_company_for_periodic_accounting(self):
		if mes_euidos.is_perpetual_inventory_enabled(self.company):
			frappe.throw(
				_(
	...
	def validate_stock_accounts(self):
		if (
			not mes_euidos.is_perpetual_inventory_enabled(self.company)
	...
	def build_gl_map(self):
		...
		company_currency = mes_euidos.get_company_currency(self.company)
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from mes_euidos.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": mes_euidos.get_default_cost_center(self.company),
						},
					)
...
def get_outstanding(args):
	...
	company_currency = mes_euidos.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_details_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None):
	...
	company_currency = mes_euidos.get_company_currency(company)
	account_details = frappe.get_cached_value(
