...
frappe.provide("mes_euidos.journal_entry");
...
frappe.ui.form.on("Journal Entry", {
	...
	refresh: function (frm) {
		if (frm.doc.reversal_of && (frm.is_new() || frm.doc.docstatus == 0)) {
			mes_euidos.journal_entry.lock_reversal_entry(frm);
		}
		...
		if (frm.doc.docstatus == 1) {
			frm.add_custom_button(
				...
				function () {
					return mes_euidos.journal_entry.reverse_journal_entry(frm);
				},
				__("Actions")
			);
		}
		...
		if (frm.doc.__islocal) {
			frm.add_custom_button(__("Quick Entry"), function () {
				return mes_euidos.journal_entry.quick_entry(frm);
			});
		}

		// hide /unhide fields based on currency
		mes_euidos.journal_entry.toggle_fields_based_on_currency(frm);
		...
		mes_euidos.accounts.unreconcile_payment.add_unreconcile_btn(frm);

		if (frm.doc.voucher_type !== "Exchange Gain Or Loss") {
			$.each(frm.doc.accounts || [], function (i, row) {
				mes_euidos.journal_entry.set_exchange_rate(frm, row.doctype, row.name);
			});
		}
	},
	...
	make_inter_company_journal_entry: function (frm) {
		...
		d.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				...
				method: "mes_euidos.accounts.doctype.journal_entry.journal_entry.make_inter_company_journal_entry",
				callback: function (r) {
					if (r.message) {
	...
	company: function (frm) {
		...
		mes_euidos.utils.set_letter_head(frm);
	...
	voucher_type: function (frm) {
		...
		if (
			...
			if (["Bank Entry", "Cash Entry"].includes(frm.doc.voucher_type)) {
				return frappe.call({
					type: "GET",
					method: "mes_euidos.accounts.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
					args: {
						account_type:
...
mes_euidos.accounts.JournalEntry = class JournalEntry extends frappe.ui.form.Controller {
	onload() {
		this.load_defaults();
		this.setup_queries();
		mes_euidos.accounts.dimensions.setup_dimension_filters(this.frm, this.frm.doctype);
	}
	...
	setup_queries() {
		...
		me.frm.set_query("account", "accounts", function (doc, cdt, cdn) {
			return mes_euidos.journal_entry.account_query(me.frm);
		});
		...
		me.frm.set_query("party_type", "accounts", function (doc, cdt, cdn) {
			...
			return {
				query: "mes_euidos.setup.doctype.party_type.party_type.get_party_type",
				filters: {
					account: row.account,
				},
			};
		...
		me.frm.set_query("reference_name", "accounts", function (doc, cdt, cdn) {
			...
			if (jvd.reference_type === "Journal Entry") {
				...
				return {
					query: "mes_euidos.accounts.doctype.journal_entry.journal_entry.get_against_jv",
					filters: {
						account: jvd.account,
						party: jvd.party,
					},
	...
	get_outstanding(doctype, docname, company, child) {
		...
		return frappe.call({
			method: "mes_euidos.accounts.doctype.journal_entry.journal_entry.get_outstanding",
			args: { args: args },
			callback: function (r) {
				if (r.message) {
	...
	accounts_add(doc, cdt, cdn) {
		...
		mes_euidos.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "accounts");
	}
};

cur_frm.script_manager.make(mes_euidos.accounts.JournalEntry);
...
frappe.ui.form.on("Journal Entry Account", {
	party: function (frm, cdt, cdn) {
		...
		if (!d.account && d.party_type && d.party) {
			...
			return frm.call({
				method: "mes_euidos.accounts.doctype.journal_entry.journal_entry.get_party_account_and_currency",
				child: d,
				args: {
					company: frm.doc.company,
	...
	account: function (frm, dt, dn) {
		mes_euidos.journal_entry.set_account_details(frm, dt, dn);
	},

	debit_in_account_currency: function (frm, cdt, cdn) {
		mes_euidos.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
	...
	exchange_rate: function (frm, cdt, cdn) {
		...
		mes_euidos.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	},
});
...
$.extend(mes_euidos.journal_entry, {
	...
	set_exchange_rate: function (frm, cdt, cdn) {
		...
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			frappe.call({
				method: "mes_euidos.accounts.doctype.journal_entry.journal_entry.get_exchange_rate",
				args: {
		...
		var dialog = new frappe.ui.Dialog({
			...
			fields: [
				{ fieldtype: "Currency", fieldname: "debit", label: __("Amount"), reqd: 1 },
				{
					...
					options: "Account",
					get_query: function () {
						return mes_euidos.journal_entry.account_query(frm);
					},
				},
				{
					fieldtype: "Link",
	...
	reverse_journal_entry: function (frm) {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.accounts.doctype.journal_entry.journal_entry.make_reverse_journal_entry",
			frm: frm,
		});
	},
});
...
$.extend(mes_euidos.journal_entry, {
	set_account_details: function (frm, dt, dn) {
		...
		if (d.account) {
			...
			return frappe.call({
				method: "mes_euidos.accounts.doctype.journal_entry.journal_entry.get_account_details_and_party_type",
				args: {
					account: d.account,
				...
				callback: function (r) {
					if (r.message) {
						$.extend(d, r.message);
						mes_euidos.journal_entry.set_amount_on_last_row(frm, dt, dn);
						mes_euidos.journal_entry.set_debit_credit_in_company_currency(frm, dt, dn);
						refresh_field("accounts");
					}
				},
			});
		} else {
			mes_euidos.journal_entry.clear_fields(frm, dt, dn);
		}
	},
