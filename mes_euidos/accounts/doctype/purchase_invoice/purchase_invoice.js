...
mes_euidos.accounts.payment_triggers.setup("Purchase Invoice");
mes_euidos.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
mes_euidos.accounts.taxes.setup_tax_validations("Purchase Invoice");
mes_euidos.buying.setup_buying_controller();

mes_euidos.accounts.PurchaseInvoice = class PurchaseInvoice extends mes_euidos.buying.BuyingController {
	setup(doc) {
	...
	unblock_invoice() {
		const me = this;
		frappe.call({
			method: "mes_euidos.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice",
	...
	make_comment_dialog_and_block_invoice() {
		...
		this.dialog.set_primary_action(__("Save"), function () {
			...
			frappe.call({
				method: "mes_euidos.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice",
				args: {
	...
	set_release_date(data) {
		return frappe.call({
			method: "mes_euidos.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date",
			args: data,
	...
	make_inter_company_invoice(frm) {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm,
		});
	}
	...
	make_debit_note() {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: this.frm,
		});
	}
};

cur_frm.script_manager.make(mes_euidos.accounts.PurchaseInvoice);
...
frappe.ui.form.on("Purchase Invoice", {
	...
	mode_of_payment: function (frm) {
		mes_euidos.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
