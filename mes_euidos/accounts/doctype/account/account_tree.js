...
frappe.treeview_settings["Account"] = {
	...
	filters: [
		{
			fieldname: "company",
			fieldtype: "Select",
			options: mes_euidos.utils.get_tree_options("company"),
			label: __("Company"),
			render_on_toolbar: true,
			default: mes_euidos.utils.get_tree_default("company"),
			on_change: function () {
				...
				frappe.call({
					method: "mes_euidos.accounts.doctype.account.account.get_root_company",
					args: {
						company: company,
					},
					callback: function (r) {
	...
	root_label: "Accounts",
	get_tree_nodes: "mes_euidos.accounts.utils.get_children",
	on_node_render: function (node, deep) {
		...
		if (!cur_tree.account_balance_data) {
			frappe.db.get_single_value("Accounts Settings", "show_balance_in_coa").then((value) => {
				if (value) {
					frappe.call({
						method: "mes_euidos.accounts.utils.get_account_balances_coa",
						args: {
	...
	add_tree_node: "mes_euidos.accounts.utils.add_ac",
	menu_items: [
		{
		...
		},
		{
			...
			click: function (node, btn) {
				frappe.route_options = {
					from_date: mes_euidos.utils.get_fiscal_year(frappe.datetime.get_today(), true)[1],
					to_date: mes_euidos.utils.get_fiscal_year(frappe.datetime.get_today(), true)[2],
					company:
