...
frappe.ui.form.on("Timesheet", {
	setup: function (frm) {
		frappe.require("/assets/mes_euidos/js/projects/timer.js");
			...
			frm.add_custom_button(__(button), function () {
				...
				$.each(frm.doc.time_logs || [], function (i, row) {
					...
					if (flag && row.activity_type && !row.from_time) {
						mes_euidos.timesheet.timer(frm, row);
					...
					if (flag && row.from_time <= frappe.datetime.now_datetime() && !row.completed) {
						...
						mes_euidos.timesheet.timer(frm, row, timestamp);
						flag = false;
					}
				});
				// If no activities found to start a timer, create new
				if (flag) {
					mes_euidos.timesheet.timer(frm);
				}
	...
	make_invoice: function (frm) {
		...
		dialog.set_primary_action(__("Create Sales Invoice"), () => {
			...
			return frappe.call({
				type: "GET",
				method: "mes_euidos.projects.doctype.timesheet.timesheet.make_sales_invoice",
				args: {
		...
		if (frm.doc.employee && frm.doc.time_logs) {
			...
			frm.doc.time_logs.forEach((row) => {
				if (row.activity_type) {
					frappe.call({
						method: "mes_euidos.projects.doctype.timesheet.timesheet.get_activity_cost",
						args: {
