...
frappe.ui.form.on("BOM", {
	setup(frm) {
		...
		frm.phantom_bom_filters = {
			query: "mes_euidos.manufacturing.doctype.bom.bom.item_query",
			filters: {
	...
	make_work_order(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items, use_multi_level_bom) => {
				frappe.call({
					method: "mes_euidos.manufacturing.doctype.work_order.work_order.make_work_order",
					args: {
						bom_no: frm.doc.name,
						item: item,
						qty: data.qty || 0.0,
	...
	make_variant_bom(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items) => {
				frappe.call({
					method: "mes_euidos.manufacturing.doctype.bom.bom.make_variant_bom",
					args: {
	...
	make_quality_inspection(frm) {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm,
		});
	},
		...
		if (frm.doc.routing && frm.doc.with_operations) {
			frappe.call({
				...
				callback(r) {
					if (!r.exc) {
						frm.refresh_fields();
						mes_euidos.bom.calculate_op_cost(frm.doc);
						mes_euidos.bom.calculate_total(frm.doc);
					}
				},
			});
		}
...
mes_euidos.bom.BomController = class BomController extends mes_euidos.TransactionController {
	conversion_rate(doc) {
		if (this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			mes_euidos.bom.update_cost(doc);
		}
	}
...
extend_cscript(cur_frm.cscript, new mes_euidos.bom.BomController({ frm: cur_frm }));

cur_frm.cscript.hour_rate = function (doc) {
	mes_euidos.bom.calculate_op_cost(doc);
	mes_euidos.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return frappe.call({
			...
			callback: function (r) {
				...
				mes_euidos.bom.calculate_rm_cost(doc);
				mes_euidos.bom.calculate_total(doc);
			},
			freeze: true,
		});
...
mes_euidos.bom.update_cost = function (doc) {
	mes_euidos.bom.calculate_op_cost(doc);
	mes_euidos.bom.calculate_rm_cost(doc);
	mes_euidos.bom.calculate_total(doc);
};

mes_euidos.bom.calculate_op_cost = function (doc) {
...
// rm : raw material
mes_euidos.bom.calculate_rm_cost = function (doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
...
mes_euidos.bom.calculate_total = function (doc) {
...
frappe.ui.form.on("BOM Item", "items_remove", function (frm) {
	mes_euidos.bom.calculate_rm_cost(frm.doc);
	mes_euidos.bom.calculate_total(frm.doc);
});
