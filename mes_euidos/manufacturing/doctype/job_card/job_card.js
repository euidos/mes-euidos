...
frappe.ui.form.on("Job Card", {
	setup(frm) {
		frm.set_query("operation", () => ({
			query: "mes_euidos.manufacturing.doctype.job_card.job_card.get_operations",
		...
		if (frm.doc.docstatus === 1 && frm.doc.for_quantity > frm.doc.manufactured_qty) {
			frm.add_custom_button(__("Make Subcontracting PO"), () => {
				frappe.model.open_mapped_doc({
					method: "mes_euidos.manufacturing.doctype.job_card.job_card.make_subcontracting_po",
					frm: frm,
				});
			}).addClass("btn-primary");
		}
	...
	make_corrective_job_card(frm, operation, for_operation) {
		frappe.call({
			method: "mes_euidos.manufacturing.doctype.job_card.job_card.make_corrective_job_card",
			args: {
	...
	operation(frm) {
		...
		if (frm.doc.operation && frm.doc.work_order) {
			frappe.call({
				method: "mes_euidos.manufacturing.doctype.job_card.job_card.get_operation_details",
				args: {
	...
	make_time_log(frm, args) {
		...
		frappe.call({
			method: "mes_euidos.manufacturing.doctype.job_card.job_card.make_time_log",
			args: { args },
			freeze: true,
			callback() {
				frm.reload_doc();
	...
	make_material_request(frm) {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.manufacturing.doctype.job_card.job_card.make_material_request",
			frm: frm,
	...
	make_stock_entry(frm) {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.manufacturing.doctype.job_card.job_card.make_stock_entry",
			frm: frm,
			run_link_triggers: true,
		});
	},
