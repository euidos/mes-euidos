...
frappe.ui.form.on("Customer", {
	setup: function (frm) {
		...
		frm.make_methods = {
			Quotation: () =>
				frappe.model.open_mapped_doc({
					method: "mes_euidos.selling.doctype.customer.customer.make_quotation",
					frm: frm,
				}),
			"Sales Order": () =>
			...
			Opportunity: () =>
				frappe.model.open_mapped_doc({
					method: "mes_euidos.selling.doctype.customer.customer.make_opportunity",
					frm: frm,
				}),
			"Payment Entry": () =>
				frappe.model.open_mapped_doc({
					method: "mes_euidos.selling.doctype.customer.customer.make_payment_entry",
					frm: frm,
				}),
			"Pricing Rule": () => frm.trigger("make_pricing_rule"),
			"Bank Account": () => mes_euidos.utils.make_bank_account(frm.doc.doctype, frm.doc.name),
		};
		...
		frm.set_query("customer_primary_contact", function (doc) {
			return {
				query: "mes_euidos.selling.doctype.customer.customer.get_customer_primary",
				filters: {
					customer: doc.name,
					type: "Contact",
				},
	...
	refresh: function (frm) {
		...
		if (!frm.doc.__islocal) {
			...
			// indicator
			mes_euidos.utils.set_party_dashboard_indicators(frm);
		} else {
	...
	show_party_link_dialog: function (frm) {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function ({ supplier }) {
				frappe.call({
					method: "mes_euidos.accounts.doctype.party_link.party_link.create_party_link",
					args: {
