...
from mes_euidos.controllers.website_list_for_contact import (
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from mes_euidos.accounts.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from mes_euidos.accounts.doctype.party_account.party_account import PartyAccount
		from mes_euidos.selling.doctype.customer_credit_limit.customer_credit_limit import CustomerCreditLimit
		from mes_euidos.selling.doctype.sales_team.sales_team import SalesTeam
		from mes_euidos.selling.doctype.supplier_number_at_customer.supplier_number_at_customer import (
			SupplierNumberAtCustomer,
		)
		from mes_euidos.utilities.doctype.portal_user.portal_user import PortalUser
	...
	def copy_communication(self):
		...
		from mes_euidos.crm.utils import copy_comments, link_communications
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		if not credit_controller_role or credit_controller_role not in frappe.get_roles():
			...
			frappe.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "mes_euidos.selling.doctype.customer.customer.send_emails",
