...
import mes_euidos
from mes_euidos.accounts.general_ledger import process_gl_map
from mes_euidos.accounts.utils import get_account_currency
from mes_euidos.buying.utils import check_on_hold_or_closed_status
from mes_euidos.controllers.taxes_and_totals import init_landed_taxes_and_totals
from mes_euidos.manufacturing.doctype.bom.bom import (
	add_additional_cost,
	get_op_cost_from_sub_assemblies,
	get_secondary_items_from_sub_assemblies,
	validate_bom_no,
)
from mes_euidos.setup.doctype.brand.brand import get_brand_defaults
from mes_euidos.setup.doctype.item_group.item_group import get_item_group_defaults
from mes_euidos.stock.doctype.batch.batch import get_batch_qty
from mes_euidos.stock.doctype.item.item import get_item_defaults
...
from mes_euidos.stock.serial_batch_bundle import (
	SerialBatchCreation,
	get_empty_batches_based_work_order,
	get_serial_or_batch_items,
)
from mes_euidos.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from mes_euidos.stock.utils import get_bin, get_combine_datetime, get_incoming_rate
...
from mes_euidos.controllers.stock_controller import StockController
from mes_euidos.controllers.subcontracting_inward_controller import SubcontractingInwardController
...
class StockEntry(StockController, SubcontractingInwardController):
	...
	if TYPE_CHECKING:
		...
		from mes_euidos.stock.doctype.landed_cost_taxes_and_charges.landed_cost_taxes_and_charges import (
			LandedCostTaxesandCharges,
		)
		from mes_euidos.stock.doctype.stock_entry_detail.stock_entry_detail import StockEntryDetail
	...
	def before_validate(self):
		from mes_euidos.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def check_invalid_serial_batch_nos_for_finished_good_item(self, row) -> bool:
		from mes_euidos.stock.doctype.serial_no.serial_no import get_serial_nos
		from mes_euidos.stock.serial_batch_bundle import get_batches_from_bundle, get_serial_nos_from_bundle
	...
	def validate_difference_account(self):
		if not cint(mes_euidos.is_perpetual_inventory_enabled(self.company)):
			return
	...
	def validate_source_stock_entry(self):
		...
		from mes_euidos.manufacturing.doctype.work_order.work_order import get_disassembly_available_qty
	...
	def set_actual_qty(self):
		from mes_euidos.stock.stock_ledger import is_negative_stock_allowed
	...
	def validate_same_source_target_warehouse_during_material_transfer(self):
		...
		if frappe.get_single_value("Stock Settings", "validate_material_transfer_warehouses"):
			from mes_euidos.stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
		...
		for d in self.get("items"):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				...
				d.basic_rate = get_valuation_rate(
					...
					currency=mes_euidos.get_company_currency(self.company),
					company=self.company,
	...
	def get_backflush_based_on(self):
		from mes_euidos.manufacturing.doctype.bom.bom import get_backflush_based_on
	...
	def get_bom_raw_materials(self, qty):
		from mes_euidos.manufacturing.doctype.bom.bom import get_bom_items_as_dict
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose in ["Send to Subcontractor", "Material Transfer"]:
			from mes_euidos.subcontracting.doctype.subcontracting_order.subcontracting_order import (
	...
	def update_pick_list_status(self):
		from mes_euidos.stock.doctype.pick_list.pick_list import update_pick_list_status
...
def get_expired_batch_items():
	from mes_euidos.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_auto_batch_nos
