...
from mes_euidos.stock.doctype.item_default.item_default import ItemDefault
from mes_euidos.stock.utils import get_valuation_method
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from mes_euidos.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from mes_euidos.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from mes_euidos.stock.doctype.item_default.item_default import ItemDefault
		from mes_euidos.stock.doctype.item_reorder.item_reorder import ItemReorder
		from mes_euidos.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from mes_euidos.stock.doctype.item_tax.item_tax import ItemTax
		from mes_euidos.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from mes_euidos.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": mes_euidos.get_default_currency(),
	...
	def recalculate_bin_qty(self, new_name):
		from mes_euidos.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"mes_euidos.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
						now=frappe.in_test,
						timeout=600,
...
def get_asset_naming_series():
	from mes_euidos.assets.doctype.asset.asset import get_asset_naming_series
