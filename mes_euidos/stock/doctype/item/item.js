...
frappe.provide("mes_euidos.item");
...
frappe.ui.form.on("Item", {
	...
	onload: function (frm) {
		mes_euidos.item.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		if (frm.doc.has_variants) {
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						mes_euidos.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						mes_euidos.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						mes_euidos.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		mes_euidos.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			mes_euidos.item.make_dashboard(frm);
		}

		mes_euidos.item.render_item_prices(frm);
	...
	validate: function (frm) {
		mes_euidos.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		frappe.call({
			method: "mes_euidos.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
...
$.extend(mes_euidos.item, {
	setup_queries: function (frm) {
		...
		frm.fields_dict.reorder_levels.grid.get_field("warehouse_group").get_query = function (
			...
			let row = locals[cdt][cdn];
			return {
				query: "mes_euidos.stock.doctype.warehouse.warehouse.get_warehouses_for_reorder",
				filters: {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				mes_euidos.item.item_dashboard = new mes_euidos.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "mes_euidos.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				mes_euidos.item.item_dashboard.refresh();
			});
		}
	},
	...
	render_item_prices: function (frm) {
		...
		frappe.call({
			method: "mes_euidos.stock.doctype.item.item.get_item_prices",
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				method: "mes_euidos.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				frappe.call({
					method: "mes_euidos.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "mes_euidos.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					frappe.call({
						method: "mes_euidos.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "mes_euidos.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
					stock_uom: frm.doc.stock_uom,
				},
