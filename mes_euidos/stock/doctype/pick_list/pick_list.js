...
frappe.ui.form.on("Pick List", {
	...
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "mes_euidos.stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
				...
				frappe.prompt(
					...
					(data) => {
						...
						mes_euidos.utils.map_current_doc({
							method: "mes_euidos.manufacturing.doctype.work_order.work_order.create_pick_list",
							target: frm,
	...
	create_delivery(frm, doctype) {
		frappe.model.open_mapped_doc({
			method: "mes_euidos.stock.doctype.pick_list.pick_list.create_delivery",
			args: {
				target: doctype,
			},
			frm: frm,
	...
	create_stock_entry: (frm) => {
		frappe
			.xcall("mes_euidos.stock.doctype.pick_list.pick_list.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new mes_euidos.utils.BarcodeScanner(opts);
...
function get_item_details(item_code, uom = null, warehouse = null, company = null) {
	if (item_code) {
		return frappe.xcall("mes_euidos.stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
			warehouse,
			company,
