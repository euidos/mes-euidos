...
from mes_euidos.stock.doctype.repost_item_valuation.repost_item_valuation import get_recipients
from mes_euidos.stock.doctype.stock_reposting_settings import stock_reposting_settings as srs
...
class TestStockRepostingSettings(ERPNextTestSuite):
	...
	def test_reposts_only_current_financial_year_entries(self):
		...
		calls = []
		variance_path = "mes_euidos.stock.report.stock_ledger_variance.stock_ledger_variance.get_data"
		with (
			patch.object(
	...
	def test_value_comparison_excludes_je_and_flags_wrong_account(self):
		...
		sent = []
		comparison = (
			"mes_euidos.stock.report.stock_and_account_value_comparison.stock_and_account_value_comparison"
		)
		with (
			patch("mes_euidos.is_perpetual_inventory_enabled", return_value=True),
