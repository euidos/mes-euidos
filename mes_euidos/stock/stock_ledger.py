...
import mes_euidos
from mes_euidos.stock.doctype.bin.bin import update_qty as update_bin_qty
...
from mes_euidos.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from mes_euidos.controllers.stock_controller import future_sle_exists
...
class update_entries_after:
	...
	def process_sle(self, sle):
		...
		if not cint(mes_euidos.is_perpetual_inventory_enabled(sle.company)):
			return
	...
	def update_serial_batch_no_valuation(self, sle, doc, prev_sle=None):
		from mes_euidos.stock.serial_batch_bundle import BatchNoValuation, SerialNoValuation
	...
	def update_batched_values(self, sle):
		from mes_euidos.stock.serial_batch_bundle import BatchNoValuation
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=mes_euidos.get_company_currency(sle.company),
			company=sle.company,
		)
...
def get_valuation_rate(
	...
	if (
		...
		and cint(mes_euidos.is_perpetual_inventory_enabled(company))
	):
