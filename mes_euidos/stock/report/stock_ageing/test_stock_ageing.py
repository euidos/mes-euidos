...
from mes_euidos.stock.report.stock_ageing.stock_ageing import FIFOSlots, format_report_data, get_average_age
...
class TestStockAgeing(ERPNextTestSuite):
	...
	def test_moving_average_value_ties_to_stock_balance(self):
		...
		with patch("mes_euidos.stock.utils.get_valuation_method", return_value="Moving Average"):
			slots = FIFOSlots(self.filters, sle).generate()
	...
	def test_lifo_consumes_newest_first(self):
		...
		with patch("mes_euidos.stock.utils.get_valuation_method", return_value="LIFO"):
			slots = FIFOSlots(self.filters, sle).generate()
